...
function getTemplate(templateID, defaultUris) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            template.base = path.join(index_1.TEMP_FOLDER, template.base || defaultUris.base);
            template.index = path.join(index_1.TEMP_FOLDER, template.index || defaultUris.index);
            template.namespace = path.join(index_1.TEMP_FOLDER, template.namespace || defaultUris.namespace);
            template.type = path.join(index_1.TEMP_FOLDER, template.type || defaultUris.type);
            template.header = path.join(index_1.TEMP_FOLDER, template.header || defaultUris.header);
            template.footer = path.join(index_1.TEMP_FOLDER, template.footer || defaultUris.footer);
            template.navigation = path.join(index_1.TEMP_FOLDER, template.navigation || defaultUris.navigation);
            template.constructors = updatePath(index_1.TEMP_FOLDER, template.constructors || defaultUris.constructors);
            template.fields = updatePath(index_1.TEMP_FOLDER, template.fields || defaultUris.fields);
            template.properties = updatePath(index_1.TEMP_FOLDER, template.properties || defaultUris.properties);
            template.events = updatePath(index_1.TEMP_FOLDER, template.events || defaultUris.events);
            template.methods = updatePath(index_1.TEMP_FOLDER, template.methods || defaultUris.methods);
            template.localCss = updatePathForArray(index_1.TEMP_FOLDER, template.localCss || defaultUris.localCss);
            template.localScripts = updatePathForArray(index_1.TEMP_FOLDER, template.localScripts || defaultUris.localScripts);
            template.localFiles = updatePathForArray(index_1.TEMP_FOLDER, template.localFiles || defaultUris.localFiles);
            template.globalCss = updatePathForArray(index_1.TEMP_FOLDER, template.globalCss || defaultUris.globalCss);
            template.globalScripts = updatePathForArray(index_1.TEMP_FOLDER, template.globalScripts || defaultUris.globalScripts);
            return template;
        }
        catch (e) {
            console.log(e);
...
function getTemplateToolLocation(templateID) {
    ...
    if (!templateZip.endsWith(".zip")) {
        templateZip = templateZip + ".zip";
    }
...
function gatherUris(templatePath, template, yamlUri) {
    ...
    template.localFiles = getFilenames(template.includeDefaultFiles, basePath, yamlJson.localFiles, template.localFiles);
