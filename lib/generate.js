...
function assignTypeToSidebr(sidebar, typeInfo) {
    ...
    tempSidebar = insertionSortChild(tempSidebar, new TemplateApi_1.SidebarView(typeInfo.typeInfo.name, read_xml_1.createInternalLink(typeInfo.typeInfo.unlocalizedName), (typeInfo.isStatic ? "static," : "") + "type"));
...
function insertMember(typeId, type, sidebar, details) {
    // Variables
    let name;
    for (let i = 0; i < details.length; i++) {
        ...
        sidebar.children.push(new TemplateApi_1.SidebarView(name, `${read_xml_1.createInternalLink(type.typeInfo.unlocalizedName)}#${template_helpers_1.getIdFrom(details[i])}`, typeId));
    }
    return sidebar;
}
