...
function compileMethod(filename, details) {
    ...
    const xmlApi = getApiItems(api.get(typePath));
    fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `${typePath}\n`);
    return ejs.render(read_file_1.readFile(filename), {
        details: details,
        xmlDocs: xmlApi,
        typeInfo: generatedTypeJson.typeInfo
...
function getMethodTypePath(details) {
    ...
    fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `${typePath}.${name}(${details.parameters.join(',')})\n`);
    details.parameters.forEach(function (parameter) {
        // Variables
        let paramResult = parameter.typeInfo.nonInstancedFullName;
        let temp;
        fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `  Implemented Generics:\n`);
        for (let i = 0; i < details.implementedType.genericParameters.length; i++) {
            ...
            fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `\tType: ${temp}\n`);
            if (paramResult == temp) {
                paramResult = "`" + i;
                break;
            }
            fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `\tCurrent: ${paramResult}\n`);
            fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `\t\tRegex: ([\(<,])${temp}([\)\[>,])\n`);
            paramResult = paramResult.replace(new RegExp(`([\\(<,])${temp}([\\)\\[>,])`, "gm"), "$1`" + i + "$2");
            fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `\tUpdate 1: ${paramResult}\n`);
            fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `\t\tRegex: ${temp}((?:\[,*\])+)\n`);
            paramResult = paramResult.replace(new RegExp(`${temp}((?:\\[,*\\])+)`, "gm"), "`" + i + "$1");
            fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `\tUpdate 2: ${paramResult}\n`);
        }
        fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `  Method Generics:\n`);
        ...
        fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `\tLast Update: ${paramResult}\n`);
        fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/compiled-type-path.txt", `===============================\n`);
