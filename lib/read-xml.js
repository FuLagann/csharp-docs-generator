...
function gatherTemplateApi(args) {
    // Variables
    let content;
    const parser = new xmldom_1.DOMParser();
    let api = new Map();
    let xmls = getXmls(args.binaries);
...
exports.gatherTemplateApi = gatherTemplateApi;
    ...
    for (let i = 0; i < members.length; i++) {
        ...
        for (let mid = 0; mid < args.length; mid++) {
            if (!temp.has(args[mid])) {
                temp.set(args[mid], new Map());
            }
            temp = temp.get(args[mid]);
