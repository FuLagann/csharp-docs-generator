...
function getTextContent(member, id, defaultText) {
    ...
    io.mkdirP(index_1.TEMP_FOLDER + "debugging/");
    fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/debug.txt", "\tInner: " + elems[0].innerHTML + "\n");
    fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/debug.txt", "\tOutter: " + elems[0].outerHTML + "\n");
    if (elems[0].hasChildNodes()) {
        for (let a = 0; a < elems[0].childNodes.length; a++) {
            fs.appendFileSync(index_1.TEMP_FOLDER + "debugging/debug.txt", "\t\tChild: " + elems[0].childNodes[a].textContent + "\n");
        }
    }
    if (desc.endsWith("..")) {
        desc = desc.substring(0, desc.length - 1);
