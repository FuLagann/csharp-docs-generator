...
function makeTextContentFriendly(desc) {
    ...
    const results = desc.replace(pattern, function (substring, args) {
        fs.appendFileSync(index_1.TEMP_FOLDER + "debug.txt", "Text: " + substring + "\n");
        fs.appendFileSync(index_1.TEMP_FOLDER + "debug.txt", "\t" + args[0] + "\n");
        fs.appendFileSync(index_1.TEMP_FOLDER + "debug.txt", "\t" + args[1] + "\n");
        fs.appendFileSync(index_1.TEMP_FOLDER + "debug.txt", "\t" + args[2] + "\n");
        fs.appendFileSync(index_1.TEMP_FOLDER + "debug.txt", "\t---------------\n");
        return substring;
    });
    return results;
}
